## explicit_burgers_periodic_2d.py
...
diffusion) in two dimensions with periodic boundaries.
"""
import glob
import numpy as np
...
for cycles in xrange(my_fluid.cycle_start, my_fluid.NT+1):
    ...
    u[1:-1,1:-1] += (my_fluid.advect_explicit_CFD_2dX(u_old, v_old)
                  +  my_fluid.diffuse_CFD_2d(u_old))

    v[1:-1,1:-1] += (my_fluid.advect_explicit_CFD_2dY(u_old, v_old)
...
## end explicit_burgers_periodic_2d.py
