...
if my_fluid.cycle_start == 1:
    ...
    A   = np.zeros((my_fluid.NX,my_fluid.NY)) # magnet vector potential
...
for cycles in xrange(my_fluid.cycle_start, my_fluid.NT+1):
    ...
    u[1:-1,1:-1]  = (my_fluid.nonlinear_advect_implicit_periodic_2d( u,
                                                            u,v, XX,YY)
                  -  my_fluid.nonlinear_advect_implicit_periodic_2d( Bx,
                                                            Bx,By, XX,YY)
                  + Bx[1:-1,1:-1]
                  -  my_fluid.apply_pressure_2dX(Bx**2+By**2, 0.5)
                    )
    v[1:-1,1:-1]  = (my_fluid.nonlinear_advect_implicit_periodic_2d( v,
                                                        u_old,v, XX,YY)
                  -  my_fluid.nonlinear_advect_implicit_periodic_2d(By,
                                                          Bx,By, XX,YY)
                  + By[1:-1,1:-1]
    ...
    A[1:-1,1:-1]  = my_fluid.nonlinear_advect_implicit_periodic_2d( A,
                                                           u,v, XX,YY)
    A[ 0, : ] = A[ -2,  : ]; A[ -1,  : ] = A[ 1, : ]
    A[ :, 0 ] = A[  :, -2 ]; A[  :, -1 ] = A[ :, 1 ]
