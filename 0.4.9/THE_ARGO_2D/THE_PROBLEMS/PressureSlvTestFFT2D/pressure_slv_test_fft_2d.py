## pressure_slv_test_fft_2d.py
## by Ryan Farber 18 March 2015
## Last modified: 04 April 2015
"""
The purpose of this program is to apply the_argo to an initialized
divergence in the velocity field and see how well it removes the
divergence with the fft pressure solver.
"""
import glob
import numpy as np
...
for cycles in xrange(my_fluid.cycle_start, my_fluid.NT+1):
    ...
    print("before src: ", absMaxSrc)
    print("before divV: ", divV)
    ...
    src[1:-1,1:-1] = my_fluid.calc_source_2d(u,v)
    absMaxSrc = sum(sum(np.absolute(src)))
    print("after src: ", absMaxSrc)
    print("after divV: ", divV)
...
## end pressure_slv_test_fft_2d.py
