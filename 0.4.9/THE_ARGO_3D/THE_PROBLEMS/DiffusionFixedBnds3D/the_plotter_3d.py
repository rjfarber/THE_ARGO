...
The Argo 3D from a saved state file and to plot the 3D variable(s).
...
for pfile in pfiles:
    ...
    if   "quiver" in my_fluid.PLT_TYP:
        if   "pcolormesh" in my_fluid.PLT_TYP:
            ...
            plt.pcolormesh(X,Y,
                    the_data[4][ 1:-1, 1:-1, (len(the_data[4])-1)/2 ])
            plt.colorbar()

            plt.quiver(X,Y,
                    the_data[1][ 1:-1, 1:-1, (len(the_data[1])-1)/2 ],
                    the_data[2][ 1:-1, 1:-1, (len(the_data[2])-1)/2 ],
                       color='w')
            ...
            plt.quiver(X[ ::2, ::2 ], Y[ ::2, ::2 ],
                       the_data[1][ ::2, ::2, (len(the_data[1])-1)/2 ],
                       the_data[2][ ::2, ::2, (len(the_data[2])-1)/2 ])
            plt_title  = "Velocity Arrows on Pressure Contours at Cycle"
            plt_title += " " + str(the_data[0]) 
        else:
            plt.quiver(X[ ::3, ::3 ], Y[ ::3, ::3 ],
                       the_data[1][ ::3, ::3, (len(the_data[1])-1)/2 ],
                       the_data[2][ ::3, ::3, (len(the_data[2])-1)/2 ])
        ...
        if   "velocity" in my_fluid.PLT_TYP:
            zlbl = "velocity"
            var  = the_data[1][ :, :, (len(the_data[1])-1)/2 ]
            ax.set_zlim(1, 2.5)
        elif "pressure" in my_fluid.PLT_TYP:
            zlbl = "pressure"
            var  = the_data[5][ :, :, (len(the_data[5])-1)/2 ]
#            ax.view_init(30, 225)
        else:
